...
%--------------------------------------------------------------------------
    ...
    methods(Access = protected, Sealed)
        ...
        %   runtime exceeds.
        %
        %   obj.outputFcn is called here, whoszssong_teste runtime will not be counted
        ...
        %       end
            ...
            index = max(1,min(min(num,size(obj.result,1)+1),ceil(num*obj.metric.runtime/obj.pro.maxRuntime)));
            obj.result(index,:) = {obj.pro.FE,Population};
            % drawnow('limitrate');

           if obj.pro.FE >= obj.pro.maxFE
               obj.pro.FE = obj.pro.FE+1; 
                obj.testR = obj.pro.Evaluation(Population.decs);
           end
            
            obj.outputFcn(obj,obj.pro);
            nofinish = obj.pro.FE < obj.pro.maxFE;
            % nofinish = obj.metric.runtime  < obj.pro.maxRuntime;
...
%     clc; fprintf('%s on %d-objective %d-variable %s (%6.2f%%), %.2fs passed...\n',class(Algorithm),Problem.M,Problem.D,class(Problem),Problem.FE/Problem.maxFE*100,Algorithm.metric.runtime);
    if Problem.FE >= Problem.maxFE
        ...
        elseif Algorithm.save > 0
            ...
            [~,~]  = mkdir(folder);
            file   = fullfile(folder,sprintf('%s_%s_%d_M%d_D%d_',class(Algorithm),class(Problem),Problem.DataNo,Problem.M,Problem.D));
...
%             end
            ...
            name = 'HV';
            Algorithm.metric.(name) = Algorithm.CalMetric(name);
            name1 = 'MCER';
            Algorithm.metric.(name1) = Algorithm.CalMetric(name1);
            testR = Algorithm.testR;
            FrontNo = NDSort(result{end,2}.objs, inf);

            % the results in test dataset are saved in testR. 

            HV1 = HV(testR(FrontNo==1),[1 1]);
            Mcer =MCER(testR);
            Mcert =MCERT(testR,result{end,2});
            % NewCP = newCP(testR,result{end,2}, Problem);
            metric = Algorithm.metric;
            save([file,num2str(runNo),'.mat'],'result','metric','Mcert','testR','HV1','Mcer');
        end
    end
end
