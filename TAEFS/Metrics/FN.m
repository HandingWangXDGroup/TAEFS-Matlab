...
% Selected feature number with Minimal classification error rate (the 
% solution with the MCER on the training set and apply it on the tet set) 
...
%--------------------------------------------------------------------------
    ...
    score = sum(max(PopObj(:,2))).*size(Population.decs,2);
end